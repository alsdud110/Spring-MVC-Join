...
public class MemberDao {
	...
	private RowMapper<Member> memRowMapper = new RowMapper<Member>() {
			...
			public Member mapRow(ResultSet rs, int nowNum) throws SQLException{
				...
				member.setAddr_road(rs.getString("addr_road"));
				member.setAddr_detail(rs.getString("addr_detail"));
	...
	public void insert(Member member) {
		jdbcTemplate.update(new PreparedStatementCreator() {
			...
			public PreparedStatement createPreparedStatement(Connection con)
				...
				pstmt.setString(6, member.getAddr_road());
				pstmt.setString(7, member.getAddr_detail());
